<?php
	...
	$query = " 
            INSERT INTO orders(created_at, referrer, request, ip_src, item, item_id, item_price, item_params, item_count, city_area, address, fio, phone, email, owner_id, status_step1)
					   VALUES (NOW(), :referrer, :request, :ip_src, :item, :item_id, :item_price, :item_params, :item_count, :city_area, :address, :fio, :phone, :email, :owner_id, 100);								   
        "; 
	$query_params = array( 
		':item' => $sel_item['name'],
		...
		':ip_src' => $_POST['ip_src'],
		':item_id' => $_POST['id'],
		':item_price' => number_format(floatval($sel_item['price'])*intval(1),2,'.',''),
	...
	<div id="posts">
		...
		<?php if ($sel_item['finish_screen_fast']) {
		  ...
		  <FORM action="http://goodthing.in.ua/theseller/do_order_details.php" method=post name="order_form_13" target="_parent" onsubmit="return checkForm_13(this)">
				...
				<input type='hidden' id="order_id" name='order_id' value='<?php echo $new_order['id']?>'>				
